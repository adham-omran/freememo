      ...
      (dom/div
        ...
        (dom/div
          ...
          (dom/div
            ...
            ;; Context proxy — tier 1 (≤1500px) matches the inline Context
            ;; (`.toolbar-collapse-first`). Two mounts bind to the same atoms;
            ;; the atoms are the single source of truth.
            (when llm-enabled?
              (dom/div
            ...
            ;; Basic/Cloze proxy — `.toolbar-overflow-card-type` reveals at
            ;; tier 5 (one step earlier than the rest of the overflow items,
            ;; so the card-type selector goes to the dropdown before Delete /
            ;; Add new card / Generate with Prompt).
            (when llm-enabled?
              (dom/div
            ...
            (when llm-enabled?
              (dom/button
                (dom/props {:class "btn btn-sm btn-secondary toolbar-overflow-panel-action toolbar-overflow-default"
                            ...
                            :data-tooltip "Generate with Prompt"})
            ...
            (dom/button
              ...
              (dom/On "click" (fn [_]
                                ...
                                (reset! !overflow-open false)) nil))
            (dom/button
              (dom/props {:class "btn btn-sm btn-secondary toolbar-overflow-panel-action toolbar-overflow-second"
                          :aria-label "Export"
                          :data-tooltip "Export"})
              ...
              (dom/On "click" (fn [_]
                                ...
                                (reset! !overflow-open false)) nil))
            (dom/button
              (dom/props {:class "btn btn-sm btn-secondary toolbar-overflow-panel-action toolbar-overflow-early"
                          :aria-label "Pull from Anki"
                          :data-tooltip "Pull from Anki"})
          ...
          ;; Generate cluster — Generate buttons + parameters share one visual
          ...
          ;; Extract, Add, Export, Anki Sync (own internal group boundary)
