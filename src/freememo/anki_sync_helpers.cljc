...
;; ---------------------------------------------------------------------------
;; Basic & Cloze FreeMemo note types — app-owned like IO/Score. The app owns
...
#?(:cljs
   (defn ensure-basic-model!
     "Missionary task: create-or-restore the Basic FreeMemo model. Mandatory
...
#?(:cljs
   (defn ensure-cloze-model!
     "Missionary task: create-or-restore the Cloze FreeMemo model (cloze-typed
...
(defn build-cloze-note
  "AnkiConnect addNote map for a cloze card → Cloze FreeMemo. Back Extra +
...
#?(:cljs
   (defn build-note
     "Build an AnkiConnect addNote map, routing by card kind to the app-owned
      ...
      Every kind is app-owned: occlusion→IO, score→Score, cloze→Cloze FreeMemo,
      else→Basic FreeMemo."
     [card settings filename-map]
     (cond
     ...
     (let [new-cards (vec (filter #(nil? (:flashcards/anki_note_id %)) cards))
       ...
       (m/sp
         ...
         (let [all-media-ids (reduce (fn [acc card] (into acc (collect-card-media-ids card)))
               ...
               ;; Phase 1.9: read each changed basic/cloze note's current model
               ;; so Phase 2 migrates (updateNoteModel) only foreign notes and
               ...
               (if (empty? owned-changed)
                 {}
                 (into {}
                   (map (fn [n] [(:noteId n) (:modelName n)]))
                   (m/? (anki-call! "notesInfo"
                          {:notes (mapv :flashcards/anki_note_id owned-changed)}))))
                                  ...
                                  (if (= (get model-by-note note-id) owned-model)
                                    ...
                                    (m/? (anki-call! "updateNote"
                                           {:note {:id note-id :fields field-map
                                                   :tags (:tags settings)}}))
                                    ;; Foreign model — one-time lazy migration to the
                                    ;; owned model. updateNoteModel wipes omitted
                                    ;; fields, so seed the user-owned fields empty
                                    ;; (a pre-migration note has none of them to lose).
                                    (let [migrate-map (cond-> (assoc field-map "Remarks" "")
                                                        (= kind "cloze") (assoc "Back Extra" ""))]
                                      (m/? (anki-call! "updateNoteModel"
                                             {:note {:id note-id :modelName owned-model
                                                     :fields migrate-map
                                                     :tags (:tags settings)}})))))
                                ;; Occlusion/score — already owned, field-only update.
                                (m/? (anki-call! "updateNote"
                                       {:note {:id note-id :fields field-map
                                               :tags (:tags settings)}})))
